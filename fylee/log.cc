...
class CoroIdFormatItem : public LogFormatter::FormatItem {
public:
    CoroIdFormatItem(const std::string& str = "") {}
    void format(std::ostream& os, Logger::ptr logger, LogLevel::Level level, LogEvent::ptr event) override {
        os << event->getFiberId();
    }
};
...
LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level
            ...
            ,uint32_t tid, uint32_t fiber_id, uint64_t time
...
Logger::Logger(const std::string& name)
    ...
    formatter_.reset(new LogFormatter("%d{%Y-%m-%d %H:%M:%S}%T%t%T%N%T%C%T[%p]%T[%c]%T%f:%l%T%m%n"));
}
...
void Logger::setFormatter(const std::string& val) {
    ...
    fylee::LogFormatter::ptr new_val(new fylee::LogFormatter(val));
    if(new_val->isError()) {
...
void Logger::addAppender(LogAppender::ptr appender) {
    ...
    if(!appender->getFormatter()) {
        MutexType::Lock lock_(appender->mutex_);
...
fylee::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    fylee::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
    ...
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_value,
            ...
            for(auto& i : new_value) {
                ...
                for(auto& a : i.appenders) {
                    fylee::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        if(!fylee::EnvMgr::GetInstance()->has("d")) {
